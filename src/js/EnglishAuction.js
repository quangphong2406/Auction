App = {
    web3Provider: null,
    myAccount: null,
    contracts_address: "0x0930abf2b1edc9a2a27241977f1268749aaf1095",
    ...
    initContract: function () {
        ...
        App.auctionInstance.getAuctionCount.call(function (error, count) {
            if (error) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    App.auctionInstance.getTimeInfo.call(i, function (error, results) {
                        ...
                        if (!error) {
                            ...
                            // console.log(results[2]+ " " + results[2]['c'][0]);
                    ...
                    App.auctionInstance.getPriceInfo.call(i, function (error, variables) {
                        console.log("Callback price " + i.toString());
                        
                        if (!error) {
                            console.log(variables);
                            
                            $(".object_" + i.toString()).find(".currentPrice").text(variables['c'][0].toString()[0] * (10 ** variables['e']/ 1e18));
                            // $(".object_" + i.toString()).find(".currentPrice").text('10');
                        }
                        else {
                            console.log(error);
                        }
                    ...
                    App.auctionInstance.getVariables.call(i, function (error, variables) {
                        ...
                        if (!error) {
                            console.log("Name: " + App.byte2string(variables[1]));
                            console.log("Beneficiary: " + variables[0]);
                            console.log("Winners: " + variables[2]);
                            console.log(variables[3])
                            
                            $(".object_" + i.toString()).find(".panel-title").text(App.byte2string(variables[1]));
                            $(".object_" + i.toString()).find(".beneficiary").text(variables[0]);
                            $(".object_" + i.toString()).find(".winners-tooltip").attr('title', variables[2]);
                            $(".object_" + i.toString()).find(".winner-badge").text(variables[2]);
                            ...
                            if (App.myAccount == $(".object_" + i.toString()).find('.beneficiary').text()) {
                                ...
                                $(".object_" + i.toString()).find('.my-btn').text('YOU CAN NOT BID');
                            }
                            else {
                                console.log("You are not the owner of auction number " + i.toString());
                                $(".object_" + i.toString()).find('.my-btn').text('BID');
                                $(".object_" + i.toString()).find('input').attr("placeholder", "price for bid");
                                App.bindEvents(i);
                            }
                        }
                        else {
        ...
        $('.add-btn').click(function () {
            ...
            if (time <= 0){
                ...
                App.auctionInstance.creationFee.call(function (error, result) {
                    if (!error){
                        App.auctionInstance.createAuction(name, parseInt(time), 
                                                    {
                                                        gas: 300000,
                                                        from: App.myAccount,
                                                        value: result,
    ...
    eventWatch: function () {
        var priceEvent = App.auctionInstance.HighestBidIncreased();
    ...
    handleBid: function (event) {
        ...
        if (nextPrice <= currentPrice) {
            ...
            console.log("Bidding with " + (web3.toWei(nextPrice, 'ether')).toString());
            object_template.find('.my-btn').append("<i class='fa fa-spinner fa-spin'></i>");
            App.auctionInstance.bid(event.data['idx'], {
                gas: 300000,
                from: App.myAccount,
                value: web3.toWei(nextPrice, 'ether')
            },
